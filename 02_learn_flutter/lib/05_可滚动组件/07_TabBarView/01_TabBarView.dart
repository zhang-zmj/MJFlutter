...
import '../06_PageView与页面缓存/KeepAliveWrapper.dart';
...
*/
class MJTabBarViewWidget extends StatefulWidget {
  const MJTabBarViewWidget({super.key});

  @override
  State<MJTabBarViewWidget> createState() => _MJTabBarViewWidgetState();
}

// 通过mixin SingleTickerProviderStateMixin里面实现的createTicker可以通过传入一个回调方法获得一个Ticker，
// Ticker就是一个帧定时器，在执行start之后会一直在指定时间内执行回调
class _MJTabBarViewWidgetState extends State<MJTabBarViewWidget> with SingleTickerProviderStateMixin {
